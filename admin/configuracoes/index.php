...
function save_certificate_upload(string $field, string $prefix): ?string
{
    ...
    if (!in_array($extension, $allowed, true)) {
        flash('danger', 'Formato de certificado invalido para ' . $field . '. Use pfx, p12, pem, crt ou key.');
        return null;
    }
    ...
    if (!move_uploaded_file($_FILES[$field]['tmp_name'], $destination)) {
        flash('danger', 'Nao foi possivel salvar certificado: ' . $field);
        return null;
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    flash('success', 'Configuracoes atualizadas.');
...
<section class="py-5"><div class="container"><div class="row g-4"><div class="col-lg-3"><?php require dirname(__DIR__) . '/menu.php'; ?></div><div class="col-lg-9">
    <div class="panel-card bg-white p-4">
        <h1 class="h3 section-title">Configuracoes do sistema</h1>
        <form method="post" enctype="multipart/form-data" class="row g-4">
            <?= csrf_field() ?>
            <div class="col-12"><h2 class="h5">Loja virtual</h2><p class="text-secondary small">Controle se a loja realiza vendas online ou funciona apenas como catalogo para visualizacao dos produtos.</p></div>
            ...
            <div class="col-md-4"><label class="form-label">Ambiente</label><select class="form-select" name="fiscal_ambiente"><option value="homologacao" <?= app_config('fiscal.ambiente') === 'homologacao' ? 'selected' : '' ?>>Homologacao</option><option value="producao" <?= app_config('fiscal.ambiente') === 'producao' ? 'selected' : '' ?>>Producao</option></select></div>
            ...
            <div class="col-md-6"><label class="form-label">Razao social</label><input class="form-control" name="fiscal_razao_social" value="<?= e(app_config('fiscal.razao_social', '')) ?>"></div>
            <div class="col-md-4"><label class="form-label">Inscricao estadual</label><input class="form-control" name="fiscal_inscricao_estadual" value="<?= e(app_config('fiscal.inscricao_estadual', '')) ?>"></div>
            <div class="col-md-4"><label class="form-label">Inscricao municipal</label><input class="form-control" name="fiscal_inscricao_municipal" value="<?= e(app_config('fiscal.inscricao_municipal', '')) ?>"></div>
            ...
            <div class="col-md-4"><label class="form-label">Serie NF-e</label><input class="form-control" name="fiscal_serie_nfe" type="number" value="<?= e(app_config('fiscal.serie_nfe', '1')) ?>"></div>
            <div class="col-md-4"><label class="form-label">Proximo numero NF-e</label><input class="form-control" name="fiscal_proximo_numero_nfe" type="number" value="<?= e(app_config('fiscal.proximo_numero_nfe', '1')) ?>"></div>
            <div class="col-md-4"><label class="form-label">Codigo municipio IBGE</label><input class="form-control" name="fiscal_codigo_municipio" value="<?= e(app_config('fiscal.codigo_municipio', '')) ?>"></div>
            ...
            <div class="col-md-3"><label class="form-label">Numero</label><input class="form-control" name="fiscal_endereco_numero" value="<?= e(app_config('fiscal.endereco_numero', '')) ?>"></div>
            ...
            <div class="col-md-5"><label class="form-label">Municipio</label><input class="form-control" name="fiscal_endereco_municipio" value="<?= e(app_config('fiscal.endereco_municipio', '')) ?>"></div>
            ...
            <div class="col-12"><label class="form-check"><input class="form-check-input" type="checkbox" name="fiscal_reforma_tributaria_habilitada" <?= fiscal_reforma_enabled() ? 'checked' : '' ?>> Validar campos da Reforma Tributaria do Consumo (IBS/CBS/IS) no cadastro fiscal dos itens</label><div class="form-text">Use conforme cronograma de obrigatoriedade e orientacao contabil para o regime da empresa.</div></div>

            <div class="col-12 border-top pt-4"><h2 class="h5">Pagamento / Efi Bank</h2><p class="text-secondary small">Preparado para QR Code Pix e cartao de credito via SDK/API Efi.</p></div>
            ...
            <div class="col-md-4"><label class="form-check"><input class="form-check-input" type="checkbox" name="efi_cartao_habilitado" <?= efi_card_enabled() ? 'checked' : '' ?>> Habilitar cartao de credito</label></div>
            <div class="col-md-4"><label class="form-label">Ambiente</label><select class="form-select" name="efi_ambiente"><option value="sandbox" <?= app_config('efi.ambiente') === 'sandbox' ? 'selected' : '' ?>>Sandbox</option><option value="producao" <?= app_config('efi.ambiente') === 'producao' ? 'selected' : '' ?>>Producao</option></select></div>
            ...
            <div class="col-12 border-top pt-4"><h2 class="h5">E-mail / PHPMailer</h2><p class="text-secondary small">Usado para confirmacao de pedido, mudancas de status e avisos de entrega.</p></div>
            ...
            <div class="col-12"><button class="btn btn-brand">Salvar configuracoes</button></div>
        </form>
    </div>
</div></div></div></section>
